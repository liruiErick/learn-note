...
package Box2DSeparator
{
	...
	public class b2Separator
	{
		 ...
		 *
		 * 将多边形需求赋予他们的刚体形状需求,并用使用<代码>body< /代码>参数创建形状.<br/>
		 * 有一些规则必须遵循 (否则你可能会得到意想不到的结果):
		 * <ul>
		 * <li>这个类是专门为非多边形. 如果你想创建一个多边形, 你不需要使用这类 - Box2D <code>的b2PolygonShape < /code>类允许您创建多边形状与<code> setAsArray()< /code> / <code> setAsVector()< /code>方法.</li>
		 * <li>顶点必须按顺时针方向.</li>
		 * <li>没有三个相邻点应该躺在同样的线段.</li>
		 * <li>决不能有重叠的部分,没有“洞”.</li>
		 * </ul> <p/>
		 * @param body:b2Body, 新刚体形状需求将被存储.
		 * @param fixtureDef:b2FixtureDef, 包含所有属性(摩擦、密度等)的新刚体形状需求将继承.
		 * @param verticesVec:vertices, 非多边形的顶点,以顺时针方向.
		 * @param scale <code>[可选]</code> 用于您在Box2D绘制形状。scale越大,精度越好。默认值是30. 
		 *
		 * @see b2PolygonShape
		 ...
		 * </ul> 
		 *
		 * 检查是否在顶点 <code>verticesVec</code> 可以正确的分配到新的刚体形状需求 (更具体地说,它可以确保没有重叠段和顶点是按顺时针方向). 
		 * 建议你使用这个方法只用于调试,因为它可能花费更多的CPU使用率.
		 * <p/>
		 * @param verticesVec,验证顶点.
		 * @return 返回一个整数,可以有以下值:
		 * <ul>
		 * <li>0 如果顶点可以妥善处理.</li>
		 * <li>1 如果有重叠线.</li>
		 * <li>2 如果点是< b >不是< /b >以顺时针方向.</li>
		 * <li>3 如果有重叠线,<b>并且点不是</b>以顺时针方向.</li>
		 * </ul> 
		 * */
