$(function(){
	...
	// 需要展示的照片地址
	var photos = [
		{url:'img/照片1.jpg', title:'标题1'},
		{url:'img/照片2.jpg', title:'标题2'},
		{url:'img/照片3.jpg', title:'标题3'},
		{url:'img/照片4.jpg', title:'标题4'},
		{url:'img/照片5.jpg', title:'标题5'},
		{url:'img/照片6.jpg', title:'标题6'}
	]

	// 图片预览
	photo_preview($('图片容器'),photos);
});

function photo_preview($imgContainer,photos){
	...
	$closeBtn.on('vclick', function(){
		$bg.hide();
		$photoContainer.children().not($leftBtn).not($rightBtn).remove();
	});
	...
	$leftBtn.click(function(){
		...
		$num.text('images：'+(curIndex+1)+'/'+allImg+'　　'+titles[curIndex]);
		$ul.finish().animate({'margin-left': -100*curIndex+'%'});
	});
	...
	$rightBtn.click(function(){
		curIndex = Math.min(++curIndex,allImg-1);
		$num.text('images：'+(curIndex+1)+'/'+allImg+'　　'+titles[curIndex]);
		$ul.finish().animate({'margin-left': -100*curIndex+'%'});
	});
	...
	$imgBtn.click(function() {
		if (!isInit) {
			...
			if (photos) {
				...
				$imgBtn.each(function(i, n) {
					titles.push($(n).closest($imgContainer.children()).text());
				});
			}
			loadImg(images);
		}
		...
		$ul.css('margin-left', -100*curIndex+'%');
		$ul.appendTo($photoContainer);
		$num.text('images：'+(curIndex+1)+'/'+allImg+'　　'+titles[curIndex]);
