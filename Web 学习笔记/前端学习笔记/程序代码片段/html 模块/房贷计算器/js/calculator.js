...
lilv_array[58][1][5]=0.0690;//商贷 1～5年 5.96%
...
lilv_array[65][1][5]=0.0665;//商贷 1～5年 5.96%
...
lilv_array[72][1][5]=0.0640;//商贷 1～5年 5.96%
...
/*	//先清空月还款数下拉框
	...
	// 结果页面滑入
	$('#fjjsq .content form').finish().animate({'margin-left':-$('body').width()});
	$("#fjjsq .tab").hide();
}

$(function(){
	var $tab=$("#fjjsq .tab li");
	var $form=$("#fjjsq .content form");
	$tab.click(function(){
		$tab.removeClass("now");
		$(this).addClass("now");
		
		$form.removeClass("hidden");
		$form.eq(1-$(this).index()).addClass("hidden");
	});
	
	$('#fjjsq .back').click(function(){
		$('#fjjsq .content form').finish().animate({'margin-left':0});
		$("#fjjsq .tab").show();
	});
	
	// 组合型初始化隐藏
	$('#calc1_zuhe').css('display','none');
	$('#calc2_zuhe').css('display','none');
	
	// 计算方式切换
	var $calc1_radio1 = $('#calc1_radio1');
	var $calc1_radio2 = $('#calc1_radio2');
	var $calc2_radio1 = $('#calc2_radio1');
	var $calc2_radio2 = $('#calc2_radio2');
	var $mianji1 = $('#calc1_js_div1');
	var $zonge1 = $('#calc1_js_div2');
	var $mianji2 = $('#calc2_js_div1');
	var $zonge2 = $('#calc2_js_div2');
	$zonge1.css('display','none');
	$zonge2.css('display','none');
	
	$calc1_radio1.change(function(){
		$mianji1.css('display','block');
		$zonge1.css('display','none');
	})
	$calc1_radio2.change(function(){
		$mianji1.css('display','none');
		$zonge1.css('display','block');
	})
	$calc2_radio1.change(function(){
		$mianji2.css('display','block');
		$zonge2.css('display','none');
	})
	$calc2_radio2.change(function(){
		$mianji2.css('display','none');
		$zonge2.css('display','block');
	})
});