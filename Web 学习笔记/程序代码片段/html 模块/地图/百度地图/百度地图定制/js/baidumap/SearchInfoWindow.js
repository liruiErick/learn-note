  ...
  /**
    ...
    baidu.object.extend(SearchInfoWindow.prototype,{
        ...
        _initPanelTemplate: function() {
            if (!this.dom.panel) {
                this.dom.panel = this._map.panel;
            }
        ...
        _initService: function () {
			...
			function searchFinish() {
				...
				evt.initEvent("searchFinish",true);
            ...
            if (!this.localSearch) {
                this.localSearch = new BMap.LocalSearch(map, {
                    ...
                    , onSearchComplete : function (result) {
                        me._clearAddress();
                        //me._drawCircleBound();
						searchFinish(); //广播搜索结束事件
                    }
                });
            } else {
				this.localSearch.setSearchCompleteCallback(function(result) {
					me._clearAddress();
                    //me._drawCircleBound();
					searchFinish(); //广播搜索结束事件
				});
			...
			/////////////////////  自定义周边结果页面  //////////////////////////////
			this.localSearch.setResultsHtmlSetCallback(function(container){
            ...
            if (!this.transitRoute) {
				...
				/////////////////////  自定义公交车结果页面  //////////////////////////////
				this.transitRoute.setResultsHtmlSetCallback(function(container){
					var $container = $(container);
					$container.find(">div:last>a").remove();
					var $tr = $container.find("table tr");
					$tr.find("td:first").prepend("路线 ");
					$tr.find("td:last").find("div:first").prepend("<span style='color:#EB209D'>预计耗时：</span>");
					$tr.find("td:last").find("div:last").prepend("<span style='color:#EB209D'>行驶路线：</span>");
					
					// 将终点 div 移动到上方，并将终点内容改为终点实际名称
					var $div_start = $container.children("div:eq(0)");
					var $div_end = $container.children("div:eq(1)");
					$div_start.find("span").css({"overflow":"hidden","text-overflow":"ellipsis","white-space":"nowrap","display":"block"});
					$div_end.find("span").css({"overflow":"hidden","text-overflow":"ellipsis","white-space":"nowrap","display":"block"});
					$div_start.after($div_end);
					if ($div_start.text() == "起点") $div_start.find("span").text(this.name);
					if ($div_end.text() == "终点") $div_end.find("span").text(this.name);
            ...
            if (!this.drivingRoute) {
				...
				/////////////////////  自定义驾车结果页面  //////////////////////////////
				this.drivingRoute.setResultsHtmlSetCallback(function(container){
					var $container = $(container);
					$container.find(">div:last>a").remove();
					var $tr = $container.find("table tr");
					$tr.each(function(index, element) {
						$(element).find("td:first").text("第 "+(index+1)+" 步");
					});
