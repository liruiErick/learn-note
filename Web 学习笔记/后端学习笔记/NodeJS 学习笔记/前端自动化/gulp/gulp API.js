...
gulp.task('testLess', function () {
    ...
    return gulp.src(['less/**/*.less','!less/{extend,page}/*.less'])
        .pipe(less())
        .pipe(gulp.dest('./css'));
});
...
// 3.4、fn：    类型(必填)：Function 任务的执行代码；

gulp.task('testLess', function (cb) {
    // return 很重要，因为 gulp 会返回一个 promise 对象，它会确保任务代码执行完毕后，该任务才会被标记为 finished
    // 如果需要并行多个 gulp 操作，可以使用一个 promise 对象，或者在确定任务结束后手动执行 cb 函数
...
// 注意，依赖任务必须return处理结果，也就是一个steam，否则就只是优先执行依赖任务，而并不会等待依赖任务完成才执行该任务。
